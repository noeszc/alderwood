import { doesNodeContainClick } from 'client/components/lib';
...
describe('doesNodeContainClick', () => {
  ...
  describe('e.target', () => {
    it('returns node.contains(e.target) if exists', () => {
      ...
      expect(node.contains).not.toBeCalled();
  ...
  describe('nil event properties', () => {
    it('returns false if the e.clientX is nil', () => {
      ...
      expect(doesNodeContainClick(makeNode(), { clientX: undefined })).toBe(
        false,
      );
    });
    ...
    it('returns false if the e.clientY is nil', () => {
      ...
      expect(doesNodeContainClick(makeNode(), { clientY: undefined })).toBe(
        false,
      );
    });
    ...
    it('does not call node.getClientRects if e.clientX is nil', () => {
      ...
      expect(node.getClientRects).not.toBeCalled();
    });
  ...
  describe('invisible node', () => {
    it('returns false if there is no node.offsetWidth', () => {
      ...
      /* doesNodeContainClick(makeNode(null, { offsetWidth: 0 }), e).should.equal(
        false,
      ); */

      expect(doesNodeContainClick(makeNode(null, { offsetWidth: 0 }), e)).toBe(
        false,
      );

      expect(
        doesNodeContainClick(makeNode(null, { offsetWidth: null }), e),
      ).toBe(false);

      expect(
        doesNodeContainClick(makeNode(null, { offsetWidth: undefined }), e),
      ).toBe(false);
    });
    it('returns false if there is no node.offsetHeight', () => {
      const e = makeEvent();

      expect(doesNodeContainClick(makeNode(null, { offsetHeight: 0 }), e)).toBe(
        false,
      );

      expect(
        doesNodeContainClick(makeNode(null, { offsetHeight: null }), e),
      ).toBe(false);

      expect(
        doesNodeContainClick(makeNode(null, { offsetHeight: undefined }), e),
      ).toBe(false);
    });
    ...
    it('returns false if there is node.getClientRects is empty', () => {
      ...
      expect(
        doesNodeContainClick(makeNode(null, { getClientRects: () => [] }), e),
      ).toBe(false);
  ...
  describe('click outside node rect', () => {
    it('returns false if clientY < node top', () => {
      expect(
        doesNodeContainClick(makeNode({ top: 1 }), makeEvent({ clientY: 0 })),
      ).toBe(false);
    });
    ...
    it('returns false if clientX < node left', () => {
      expect(
        doesNodeContainClick(makeNode({ left: 1 }), makeEvent({ clientX: 0 })),
      ).toBe(false);
    });
    it('returns false if clientX > node right', () => {
      expect(
        doesNodeContainClick(makeNode({ right: 0 }), makeEvent({ clientX: 1 })),
      ).toBe(false);
    });
  });
