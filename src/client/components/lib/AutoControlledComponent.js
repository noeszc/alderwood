...
class AutoControlledComponent extends React.Component {
  constructor(...args) {
    super(...args);

    const { autoControlledProps } = this.constructor;
    const state =
      _.invoke(this, 'getInitialAutoControlledState', this.props) || {};
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      if (!autoControlledProps) {
        console.error(
          `Auto controlled ${name} must specify a static autoControlledProps array.`,
        );
      }
      ...
      const illegalDefaults = _.intersection(
        autoControlledProps,
        _.keys(defaultProps),
      );
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const newState = autoControlledProps.reduce((acc, prop) => {
      ...
      const propWasRemoved =
        !_.isUndefined(this.props[prop]) && isNextUndefined;
      ...
      // reinitialize state for props just removed / set undefined
        ...
        acc[prop] = getAutoControlledStateValue(prop, nextProps);
